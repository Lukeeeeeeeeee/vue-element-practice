...
export default function treeToArray(data, expandAll, parent, level, item) {
    ...
    Array.from(data).forEach(function (record) {
        ...
        if (parent) {
            ...
            if (!marTemp[_level]) {
                marTemp[_level] = 0 // E.g: marTemp = [ , 0]
            }

            Vue.set(record, '_marginLeft', marTemp[_level] + parent._marginLeft)
            Vue.set(record, '_width', record[item] / parent[item] * parent._width)

            // 在本次计算过偏移量后加上自己的长度，以供下一个元素使用
            marTemp[_level] += record._width
        } else {
            // 如果为根元素
            // 初始化偏移量，存储map
            marTemp[record.id] = [] // E.g: marTemp = [ , []]
            // map中是一个数组，存储的是每级的长度和
            // 初始为0
            marTemp[record.id][_level] = 0 // E.g: marTemp = [ , [ , 0]]
